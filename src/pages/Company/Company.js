...
const Company = () => {
  ...
  const [proDetailslength, setProDetailslength] = useState();
  const [workerDetailslength, setWorkerDetailslength] = useState();
  const [operationTeamDetailslength, setOperationTeamDetailslength] = useState();
  ...
  const fetchUsersDetails = () => {
    ...
    axios.get(`${API_URL}/main/prodetails/${companyId}/`, {
      ...
      .then(response => {
        setProDetailslength(response.data);
        setRoleCountslength(prevState => ({
          ...prevState,
          Pro: response.data.length // Update Pro count with the length of the array
        }));
      })
      .catch(error => {
    ...
    axios.get(`${API_URL}/main/operationteam/${companyId}/`, {
      ...
      .then(response => {
        setOperationTeamDetailslength(response.data);
        setRoleCountslength(prevState => ({
          ...prevState,
          Operationteam: response.data.length // Update Operationteam count with the length of the array
        }));
      })
      .catch(error => {
    ...
    axios.get(`${API_URL}/main/workerdetails/${companyId}/`, {
      ...
      .then(response => {
        setWorkerDetailslength(response.data);
        setRoleCountslength(prevState => ({
          ...prevState,
          Worker: response.data.length // Update Worker count with the length of the array
        }));
      })
      .catch(error => {
      ...
      <Sidebar>
        ...
        <div className="admin-boxes">
          <div className="admin-box">
            <h3>Total</h3>
            <span className="admin_box_count">{Object.values(roleCountslength).reduce((a, b) => a + b, 0)}</span>
          </div>
          {Object.entries(roleCountslength).map(([role, count]) => (
